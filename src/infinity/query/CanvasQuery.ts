...
import { type Signal, effect, signal, Manager, events } from '../../state'
import { isAsyncGeneratorFunction, isNotNullish } from '../../type/guards'
import { entries } from '../../type/object'
import { arraysEquals } from '../../type/equals'
import {
  type QueryParams,
  type Query,
  type QueryAPI,
...
export class CanvasQuery<Item extends any = any> extends Manager implements QueryAPI<Item> {
  private readonly entities = new Map<ID, Item>()
  private readonly queryQueue = new Map<QueryIdentifier, Query<Item>>()
  public readonly queue = this.use(events<Record<QueryIdentifier, QueryResult>>())
  private readonly data = this.use(events<Record<ID, Item | undefined>>())
  public readonly processing = this.use(signal(false))
  public readonly ids = this.use(
    signal<ID[]>([], {
  ...
  public signalQuery = <Q extends QueryParams<Item>>(
    id: QueryIdentifier,
    box: Signal<Q>,
    throttle: number = 90
  ): Signal<QueryResult> =>
    this.system.unique(id, () => {
      const visible = this.system.use(
        signal<QueryResult>(createQueryResult, {
