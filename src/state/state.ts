...
export const context = () => {
  ...
  return {
     ...
     */
    state: <V>(
      fn: V | ((use: UseStateDependency) => V),
      options: StateOptions<V> = {}
    ): State<V> => createState<V>(register(), fn, options),
  };
};
...
const createState = <V>(
  ...
  const shouldThrottle = () =>
    throttle && performance.now() - lastSyncTime < throttle;
  ...
  const set = (
    ...
    const newValue =
      shouldMerge && isObject(value) ? (merge(value, next) as V) : (next as V);
