...
export type Settable<V extends any = any> = {
  set: (
    partial: V | Partial<V> | ((state: V) => V | Partial<V>),
    sync?: boolean
  ) => void;
};
...
export type UseStateDependency = <S extends Subscribable & Gettable>(
  u: S
) => SubscribableType<S>;
...
export type Events<
  S extends EventsMap,
  K extends keyof S = EventsKey & keyof S,
> = {
  on: <Key extends K>(
